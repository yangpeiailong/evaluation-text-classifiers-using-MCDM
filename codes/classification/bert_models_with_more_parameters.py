...
class BaseBert(nn.Module):
    def __init__(self):
        ...
        self.relu = nn.ReLU()
        self.linear2 = nn.Linear(200, n_class)
    ...
    def forward(self, X):
        ...
        # logits = self.softmax(self.linear1(self.dropout(outputs.pooler_output)))
        # logits = self.linear(self.dropout(outputs.pooler_output))
        outputs = self.linear1(outputs.pooler_output)
...
class BertLSTM(nn.Module):
    def __init__(self):
        ...
        self.f1 = nn.Sequential(nn.Linear(hidden_size_lstm * 2, 200),
                                nn.Dropout(0.8),
                                nn.ReLU(),
...
class BertLSTMAttention(nn.Module):
    def __init__(self):
        ...
        # 定义计算注意力权重的内容
        self.linear1 = nn.Linear(hidden_size_lstm * 2, 200)
        self.tanh = nn.Tanh()
...
if __name__ == '__main__':
    ...
    with open('model/label_to_index_bert2_for_' + dataname + '_' + num_of_sample + '.txt', 'w', encoding='utf-8') as f:
        for i in label2ind.keys():
            f.writelines(str(i) + ':' + str(label2ind[i]) + '\n')
        f.close()
    labels = [label2ind[label] for label in labels]

    # with open('model/label_to_index_bert2_for_' + dataname + '.txt', 'w', encoding='utf-8') as f:
    #     for i in label2ind.keys():
    #         f.writelines(str(i) + ':' + str(label2ind[i]) + '\n')
    #     f.close()
    # labels = [label2ind[label] for label in labels]
    ...
    window_sizes = [2, 3, 4, 5]  # 卷积核尺寸
    hidden_size_lstm = 256  # lstm的隐藏层尺寸
    # classifier_list = ['basebert2', 'bert+wa2', 'bert+cnn2', 'bert+lstm2', 'bert+lstm+att2', 'bert+lstm+cnn2']
    classifier_list = ['basebert2']
    ...
    for i in range(k):
        ...
        for j in range(len(classifier_list)):
            ...
            bc = 0
            if classifier_list[j] == 'basebert2':
                bc = BaseBert().to(device)
                optimizer = optim.Adam(bc.parameters(), lr=1e-5, weight_decay=1e-2)
            elif classifier_list[j] == 'bert+wa2':
                bc = BertWA().to(device)
                optimizer = optim.Adam(bc.parameters(), lr=1e-5, weight_decay=1e-2)
            elif classifier_list[j] == 'bert+cnn2':
                bc = BertCNN().to(device)
                optimizer = optim.Adam(bc.parameters(), lr=1e-5, weight_decay=1e-2)
            elif classifier_list[j] == 'bert+lstm2':
                bc = BertLSTM().to(device)
                optimizer = optim.Adam(bc.parameters(), lr=1e-4, weight_decay=1e-2)
                # optimizer = optim.SGD(bc.parameters(), lr=1e-7)
            elif classifier_list[j] == 'bert+lstm+att2':
                bc = BertLSTMAttention().to(device)
                optimizer = optim.Adam(bc.parameters(), lr=1e-5, weight_decay=1e-2)
            elif classifier_list[j] == 'bert+lstm+cnn2':
            ...
            for epoch in range(epoches):
                ...
                if acc > best_val_acc:
                    ...
                    # torch.save(bc.state_dict(),
                    #            'model/model_params_best_{data}_{modelname}_{k_num}.pkl'.format(
                    #                data=dataname, modelname=classifier_list[j], k_num=i))
                    best_val_acc = acc
                    best_val_auc = auc
                    best_val_mavg = mavg
                    best_val_mae = mae
            ...
            #     'model/model_params_best_{data}_{modelname}_{k_num}.pkl'.format(data=dataname,
            #                                                                                 modelname=classifier_list[
            #                                                                                     j], k_num=i)))
    ...
    for i in range(len(measures_total_avg)):
        ...
        sheet.write(i + 1, 11, measures_total_avg[i][5])
        sheet.write(i + 1, 12, measures_total_std[i][5])
    book.save('e:/pythonwork/newclassification/results/' + dataname + '_' + num_of_sample + '_bert3_criteria.xls')
    # book.save('e:/pythonwork/newclassification/results/' + dataname + '_bert2_criteria.xls')
