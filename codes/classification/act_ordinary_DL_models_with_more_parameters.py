...
if __name__ == '__main__':
    # 加载数据集
    dataname = 'yelp_review_polarity_csv (2)'
    ...
    with open('model/label_to_index_dl2_for_' + dataname + '_' + num_of_sample + '.txt', 'w', encoding='utf-8') as f:
        for i in label2ind.keys():
            f.writelines(str(i) + ':' + str(label2ind[i]) + '\n')
        f.close()
    # with open('model/label_to_index_dl2_for_' + dataname + '.txt', 'w', encoding='utf-8') as f:
    #     for i in label2ind.keys():
    #         f.writelines(str(i) + ':' + str(label2ind[i]) + '\n')
    #     f.close()
    ...
    with open('model/word_to_index_dl2_for_'+ dataname + '_' + num_of_sample +'.txt', 'w', encoding='utf-8') as f:
        for i in word_to_idx.keys():
            f.writelines(i + ':' + str(word_to_idx[i]) + '\n')
        f.close()

    # with open('model/word_to_index_dl2_for_'+ dataname + '.txt', 'w', encoding='utf-8') as f:
    #     for i in word_to_idx.keys():
    #         f.writelines(i + ':' + str(word_to_idx[i]) + '\n')
    #     f.close()
    ...
    hiddensize = 256
    epoches = 100
    classifier_list = ['random+WA2', 'FastText+WA2', 'random+CNN2', 'FastText+CNN2', 'random+LSTM2', 'FastText+LSTM2', 'random+LSTM+att2', 'FastText+LSTM+att2', 'random+LSTM+CNN2', 'FastText+LSTM+CNN2']
    # classifier_list = ['random+LSTM+att2']
    ...
    for i in range(k):
        ...
        for j in range(len(classifier_list)):
            ...
            if classifier_list[j] == 'random+WA2':
                from DeepLearningTC.simple_models2 import WordAveraging2
                model = WordAveraging2(len(word_to_idx), embed_size, numofclasses, pad_index)
                optimizer = optim.Adam(model.parameters(), lr=1e-3)
                criteon = nn.CrossEntropyLoss().cuda()
            elif classifier_list[j] == 'FastText+WA2':
                from DeepLearningTC.simple_models2 import WordAveraging2
                model = WordAveraging2(len(word_to_idx), embed_size, numofclasses, pad_index, pretrained_weight)
                optimizer = optim.Adam(model.parameters(), lr=1e-3)
                criteon = nn.CrossEntropyLoss().cuda()
            elif classifier_list[j] == 'random+CNN2':
                from DeepLearningTC.simple_models2 import TextCNN2
                model = TextCNN2(len(word_to_idx), embed_size, numoffilters, filtersizes, MAX_LENGHT, numofclasses,
                                pad_index)
                optimizer = optim.Adam(model.parameters(), lr=1e-3)
                criteon = nn.CrossEntropyLoss().cuda()
            elif classifier_list[j] == 'FastText+CNN2':
                from DeepLearningTC.simple_models2 import TextCNN2
                model = TextCNN2(len(word_to_idx), embed_size, numoffilters, filtersizes, MAX_LENGHT, numofclasses, pad_index,
                                pretrained_weight)
                optimizer = optim.Adam(model.parameters(), lr=1e-3)
                criteon = nn.CrossEntropyLoss().cuda()
            elif classifier_list[j] == 'random+LSTM2':
                from DeepLearningTC.simple_models2 import TextLSTM2
                model = TextLSTM2(len(word_to_idx), embed_size, hiddensize, numofclasses, pad_index)
                optimizer = optim.SGD(model.parameters(), lr=1e-3)
                criteon = nn.CrossEntropyLoss().cuda()
            elif classifier_list[j] == 'FastText+LSTM2':
                from DeepLearningTC.simple_models2 import TextLSTM2
                model = TextLSTM2(len(word_to_idx), embed_size, hiddensize, numofclasses, pad_index, pretrained_weight)
                optimizer = optim.SGD(model.parameters(), lr=1e-3)
                criteon = nn.CrossEntropyLoss().cuda()
            elif classifier_list[j] == 'random+LSTM+att2':
                from DeepLearningTC.simple_models2 import TextLSTMAttention2
                model = TextLSTMAttention2(len(word_to_idx), embed_size, hiddensize, numofclasses, pad_index)
                optimizer = optim.SGD(model.parameters(), lr=1e-3)
                criteon = nn.CrossEntropyLoss().cuda()
            elif classifier_list[j] == 'FastText+LSTM+att2':
                from DeepLearningTC.simple_models2 import TextLSTMAttention2
                model = TextLSTMAttention2(len(word_to_idx), embed_size, hiddensize, numofclasses, pad_index, pretrained_weight)
                optimizer = optim.SGD(model.parameters(), lr=1e-3)
                criteon = nn.CrossEntropyLoss().cuda()
            elif classifier_list[j] == 'random+LSTM+CNN2':
                from DeepLearningTC.simple_models2 import TextLSTMCNN2
                model = TextLSTMCNN2(len(word_to_idx), embed_size, numoffilters, filtersizes, MAX_LENGHT, hiddensize, numofclasses,
                                pad_index)
                optimizer = optim.Adam(model.parameters(), lr=1e-4)
                criteon = nn.CrossEntropyLoss().cuda()
            elif classifier_list[j] == 'FastText+LSTM+CNN2':
                from DeepLearningTC.simple_models2 import TextLSTMCNN2
                model = TextLSTMCNN2(len(word_to_idx), embed_size, numoffilters, filtersizes, MAX_LENGHT, hiddensize, numofclasses, pad_index,
                                pretrained_weight)
                optimizer = optim.Adam(model.parameters(), lr=1e-4)
                criteon = nn.CrossEntropyLoss().cuda()
            # optimizer = optim.Adam(model.parameters(), lr=1e-3)
    ...
    book.save('e:/pythonwork/newclassification/results/' + dataname + '_' + num_of_sample + '_dl2_criteria.xls')
    # book.save('e:/pythonwork/newclassification/results/' + dataname + '_dl2_criteria.xls')